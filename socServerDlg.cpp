...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LIST_CLINET, m_list_client);
...
BEGIN_MESSAGE_MAP(CsocServerDlg, CDialogEx)
	...
	ON_MESSAGE(WM_CLIENT_MSG_RECV, &CsocServerDlg::OnCleintMsgRecv)
...
afx_msg LRESULT CsocServerDlg::OnCleintMsgRecv(WPARAM wParam, LPARAM lParam)
{
...
afx_msg LRESULT CsocServerDlg::OnClientClose(WPARAM wParam, LPARAM lParam)
{
	...
	if(pos!=NULL)
	{
		...
		idx = m_list_client.SelectString
		(-1, (LPCTSTR)str);
